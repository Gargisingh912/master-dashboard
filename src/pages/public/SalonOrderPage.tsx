...
export default function SalonOrderPage({ organizationId: propOrgId }: { organizationId?: string }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // salon_service_categories table may not exist — skip gracefully
      const catOrder: Record<string, number> = {};
      setCategoriesOrder(catOrder);

      const { data: items, error: servicesErr } = await supabasePublic
        .from("salon_services")
        .select("id, name, price, duration_minutes, category, subcategory, image_url, image_urls, salon_service_products(id, name, price)")
        .eq("organization_id", actualOrgId)
        .eq("is_available", true);

      if (servicesErr) console.error("Failed to load services:", servicesErr);

      // Popularity: skip nested join since salon_appointment_services relationship
      // is not in the schema cache. Popular section will remain empty until the
      // relationship is set up in Supabase.
      ...
      setServices((items || []).map((item: any) => ({
        ...
        addons: (item.salon_service_products || []).map((a: any) => ({ id: a.id, name: a.name, price: a.price })),
      })));
      setLoading(false);
    };
