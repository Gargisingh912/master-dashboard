...
import { useVenueBooking, VenueMember } from "../../context/VenueBookingContext";
import { useCalendar } from "../../hooks/useCalendar";
import { User, Phone, Calendar, X, Plus, LogIn, Bell } from "lucide-react";
import {
...
export default function VenueMemberships() {
  const { members, membershipPlans, addMember, updateMember, addCheckin, loading } = useVenueBooking();
  const { addEvent } = useCalendar(); // to send reminders
  ...
  const handleRemind = async (m: VenueMember) => {
    if (!m.contact && !m.email) {
      alert("No contact details for this member to send a reminder.");
      return;
    }
    ...
    try {
      await addEvent({
        title: `Reminder for ${m.name}`,
        description: `Follow up with member. Contact: ${m.contact || ''} Email: ${m.email || ''}`,
        event_type: "reminder",
        start_date: today,
        is_all_day: true,
        category: "Warning",
      ...
      alert(`Reminder scheduled for ${m.name} in your calendar!`);
    } catch(err: any) {
  ...
  return (
    <div className="pb-24">
      <PageMeta title="Memberships | Venue Booking" description="Manage venue memberships" />

      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-black text-gray-800 dark:text-white/90">Memberships</h1>
          <p className="text-sm text-gray-500">Manage venue members, check-ins, and reminders.</p>
        </div>
      </div>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-3">
                        <button
                          ...
                          className="text-xs font-medium text-success-600 dark:text-success-400 hover:underline flex items-center gap-1"
                        ...
                        <button
                          ...
                          className="text-xs font-medium text-warning-600 dark:text-warning-400 hover:underline flex items-center gap-1"
                          title="Send Reminder"
                        >
                          <Bell size={14} /> Remind
                        </button>
              ...
              <div className="space-y-4">
                ...
                <label className="flex items-center gap-3 p-4 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Uncheck to cancel membership.</p>
                  </div>
                </label>
              </div>
